...
func routeMinute(traffic int, channels []Channel) (int, int) {
	...
	for remaining > 0 {
		...
		minIndex := -1
		minLoad := 1000000 // велике число для пошуку мінімуму
		...
		// обраний канал (використовуємо покажчик, щоб змінити оригінал)
		ch := &channels[minIndex]

		free := ch.capacity - ch.currentLoad

		// визначаємо, скільки можемо взяти
		take := remaining
		if take > free {
			take = free
		}
...
func simulateRouting(traffic []int, channels []Channel, maxCapacity int) RoutingStats {
	...
	for _, ch := range channels {
		...
		if ch.currentLoad > ch.capacity || ch.overloaded {
...
func printReport(stats RoutingStats, channels []Channel) {
	...
	fmt.Println() // порожній рядок перед висновком
	...
	} else if stats.overloadedChannels > 0 {
